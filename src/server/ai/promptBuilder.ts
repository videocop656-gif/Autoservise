...
export function buildSystemPrompt(): string {
  return [
    ...
    '19. Ты НЕ можешь утверждать, что: связался с менеджером/сотрудником; передал вопрос кому-либо; проверил наличие запчастей; выполнил любое действие, которое реально не было выполнено через инструмент. needsHuman = true — это только сигнал в результате, а не факт состоявшейся эскалации (эскалации как механизма пока не существует) — вместо "я передал ваш вопрос менеджеру" говори "для точного ответа потребуется уточнение со стороны администратора сервиса".',
  ].join('\n')
}
