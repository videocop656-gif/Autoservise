...
- **Channel Integration Foundation** (Prompt 16): `src/server/services/channelConnectionService.ts`/`channelMessageService.ts`/`channelCustomerService.ts` + `src/server/channels/` (the `ChannelAdapter` interface, a mock-only adapter factory, and a registry keyed by `ChannelType`). A real external account connection (`ChannelConnection`) is a distinct concept from the existing `Conversation.channel` label — the latter remains the one internal source of truth for channel type, never duplicated. Inbound messages are idempotent at the DB level (`ChannelMessage`'s `@@unique([channelConnectionId, externalMessageId])`). AI is never invoked from this pipeline.
- **Channel Operations & Delivery Foundation** (Prompt 17): `src/server/services/channelDeliveryService.ts` + `channelDeliveryRepository.ts` — the outbound delivery lifecycle (`PENDING`/`SENDING`/`SENT`/`FAILED`) for an already-existing `OUTBOUND`/`STAFF` `Message`, tracked in the new `ChannelDelivery` table. Concurrent-send protection is a plain conditional `updateMany` compare-and-set (never an in-memory mutex, never `SERIALIZABLE` isolation) that works correctly across multiple serverless instances; retry reuses the same row (`attemptCount` increments, no second row is ever created); a `SENT` delivery is idempotent (the adapter is never called twice). `ChannelAdapter.sendMessage()` now takes a `NormalizedOutboundMessage` (never `tenantId`/`businessId`/client-supplied ids) and still never touches Prisma. Adapter errors are normalized to a safe `{errorCode, errorMessage, retryable}` shape — never a raw provider error or stack trace.
- **Real Telegram Channel Integration** (Prompt 18): the first real external provider connected — `src/server/channels/adapters/telegramAdapter.ts` (a real `ChannelAdapter`, still never touching Prisma) + `telegramApiClient.ts` (the one place that calls `api.telegram.org`, over the platform's native `fetch`) + `telegramSetupService.ts` (bot verification/webhook registration) + a new production-shaped `POST /api/webhooks/telegram/:connectionId`, authenticated only by a constant-time-compared `X-Telegram-Bot-Api-Secret-Token`, distinct from the still-untouched Prompt 16 foundation endpoint. `TELEGRAM_BOT_TOKEN`/`TELEGRAM_WEBHOOK_SECRET` are server-side-only env vars, never persisted, never returned, never logged. Real or mock dispatch is decided purely by whether a token is configured — the same convention `aiProviderFactory.ts` already established for `OPENAI_API_KEY` — so every existing mock-based test and the WEBSITE/WHATSAPP foundations are completely unaffected. Zero new Prisma models; AI is still never invoked from either the inbound or outbound pipeline.

- **Frontend**: React **18.3.1** (not 19), TypeScript, Vite, Tailwind CSS **v3.4.13** (not v4). No `shadcn/ui` CLI/package is installed — the UI components under `src/components/ui/` are hand-rolled in the shadcn visual style, built on `@radix-ui/react-label`, `@radix-ui/react-slot`, `class-variance-authority`, and `tailwind-merge`. `lucide-react` for icons, `react-router-dom` v6 for routing. **Design system (Prompt 19)**: "Dark Graphite + Premium Gold" — a single, deliberately-dark theme (no light/dark toggle) defined entirely as CSS variables in `src/index.css` (`background`/`card`/`primary` (gold)/`muted`/`accent`/`destructive`/`border`/`radius`, plus `success`/`warning`/`info` for system states only). **App Shell (Prompt 19)**: `src/components/layout/` — `AppShell`/`Sidebar`/`SidebarNav`/`Header`/`MobileNav`/`PageContainer`/`PageHeader`/`UserMenu`/`navigation.ts` — mounted once as a router layout route and applied to the entire authenticated app, not just new pages; gold is reserved for the active-nav indicator and primary actions, never a background.
...
**Prompt 14 (Dashboard / Analytics) deliberately added zero new Prisma
models and zero migrations** — confirmed as this stage's preferred,
spec-mandated outcome. `analyticsRepository.ts` reads the existing tables
above directly, aggregating in Postgres at request time; no denormalized
counter table, no materialized view, no pre-aggregated analytics store of
any kind exists in this schema.

**Prompt 15 (Team Management) confirmed the prediction above** — it reused
the existing `User`/`UserRole` model rather than adding a new one. The
only schema change was one additive column: `User.isActive Boolean
@default(true)` (migration `20260909075339_team_management_user_status`).
`User` still has no `businessId` column — team members belong to the
...
**Prompt 18 (Real Telegram Channel Integration) added zero new Prisma
models and zero migrations** — the existing `ChannelConnection` (bot
identity via `externalAccountId`, a safe `config.telegramUsername` display
value), `ChannelMessage`, `CustomerChannelIdentity`, and `ChannelDelivery`
already covered every real-Telegram requirement. The bot token and webhook
secret live only in server-side environment variables (`TELEGRAM_BOT_TOKEN`,
`TELEGRAM_WEBHOOK_SECRET`) — there is no column anywhere in this schema for
...
| **Dashboard** | Prompt 14 built a real, server-aggregated operational analytics dashboard here (period selector, KPI cards, AI/tool/escalation/appointment/service breakdowns, daily time series) | **Reset to a placeholder in Prompt 19** (`/dashboard`) — a deliberate product decision to rebuild it in the new "Dark Graphite + Premium Gold" design system as its own future stage; the backend (`analyticsService.ts`, `GET /api/dashboard`) is untouched and ready to be wired back up |
| **Customer Requests** | Customer Requests list, Customer Request detail (embedded in the edit form, incl. status history) | Implemented (`/settings/customer-requests`, linked from the Settings hub) |
| **Conversations** | Conversations list, Conversation detail (messages, send-message form, Close/Reopen, and — since Prompt 17 — a "Send via channel"/retry button + delivery status/attempt-count badge on eligible outbound messages of a channel-linked conversation) | Implemented (`/conversations` — a primary sidebar section since Prompt 19; the old `/settings/conversations` URL redirects here) |
| **CRM** | Customers, Customer profile (detail is the edit form; no separate profile page), Vehicles, Vehicle profile (same) | Customers implemented at `/clients` (primary sidebar section since Prompt 19; `/settings/customers` redirects here); Vehicles remain at `/settings/vehicles`, linked from the Settings hub |
| **Operations** | Appointments, Appointment detail, Service History, Service Record detail | Appointments implemented at `/appointments` (primary sidebar section since Prompt 19; `/settings/appointments` redirects here); Service History remains at `/settings/service-history`, linked from the Settings hub |
| **Configuration** | Services, Knowledge Base, Business Rules, Business Settings, Working Hours | Implemented (`/settings/services`, `/settings/knowledge`, `/settings/rules`, `/settings/business`, `/settings/hours`) — all reachable from the `/settings` hub page (Prompt 19) |
| **Users / Team** | Team list (name/email/role/status/created/actions), create form (role options limited by the current user's own role), profile edit, role-change, activate/deactivate | Implemented (`/settings/team`, Prompt 15, linked from the Settings hub) — invitations (an email-based flow rather than an admin setting a password directly) remain **Future** |
| **Channels** | Channel list (type/display name/external account/status/created/actions), create form (no credential field of any kind), edit, activate/deactivate; Telegram rows get a "Connect" setup action + bot/webhook status instead of plain Activate | Implemented (`/channels` — a primary sidebar section since Prompt 19; `/settings/channels` redirects here; Telegram real-provider setup added Prompt 18) — Telegram can be a real, live connection; WhatsApp/Website remain foundation-only (no real API connected); manager sees "View only" |
| **AI Operations** | AI Core + Booking Tools test tool (select a Conversation, type a test message, Analyze, see intent/confidence/entities/draft/needsHuman, and — since Prompt 10 — any real tool executions: availability slots, or the resulting appointment's id/status/start/end) | Implemented (`/ai-admin` — the primary sidebar "AI-администратор" section since Prompt 19, the closest existing match; `/settings/ai` redirects here) — a testing tool, not an operations dashboard; tool execution is real (a clearly-labeled warning banner says so), but sending a message to the customer stays disabled |
| **AI Escalations** | Escalations list (status/priority/unassigned filters), detail (customer/conversation/assigned-staff summary, AI's reason/summary, Claim/Resolve/Cancel) | Implemented (`/escalations` — a primary sidebar section ("Передача сотруднику") since Prompt 19; `/settings/escalations` redirects here; Prompt 12) — an internal staff workflow tool, not a customer-facing feature; no external notification of any kind |
...
**Prompt 18 connects one real wire into this diagram — Telegram, both
directions — without changing the diagram's shape at all**: the same
`ChannelConnection` → inbound pipeline → `Messages`/`Conversations` →
`channelDeliveryService.ts` → outbound flow now runs against the real
Telegram Bot API when `TELEGRAM_BOT_TOKEN` is configured (mock otherwise,
unchanged); `Phone`/`Manual` and the WhatsApp/Website mock foundations are
untouched, and — just as importantly — `AI Core`/`Tools`/`EscalationService`
still have no edge into this diagram from Telegram: a real inbound Telegram
message reaches `Messages`/`Conversations` and stops there, exactly like
