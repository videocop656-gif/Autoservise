...
`api/`/`src/` trees at the time of writing (Prompt 18 complete).
...
| **Channels** | Channel list (type/display name/external account/status/created/actions), create form (no credential field of any kind), edit, activate/deactivate; Telegram rows get a "Connect" setup action + bot/webhook status instead of plain Activate | Implemented (`/settings/channels`, Prompt 16, Telegram real-provider setup added Prompt 18) — Telegram can be a real, live connection; WhatsApp/Website remain foundation-only (no real API connected); manager sees "View only" |
